...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Benjamin Cordola, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
```{r clip-map-drought-exposure-1, warning = F}
...
I am concerned with the quality of the drought exposure data. It is essentially a population map, because although touted as a global drought risk, the units are expected average anual population (2007 as the year of reference) exposed. This raster is also double the resolution of the flood map. Therefore, I will scale the drought layer to be worth double what the flood layer is worth in the vulnerability calculation. This means drought will be worth 26.666$% and flood 13.333%, as opposed to the previous 20%and 20%.

Here, you can set which percentages you want to use for the study.

```{r set_drought_flood_percentages}

#set as 5 if you want original study result


drought_percentage = 6.666  #set at 6.666 for my change
flood_percentage = 3.333 #set at 3.333 for my change

```


Classify drought exposure into quintile classes (0 to 4)
Then rescale to 20% by multiplying by 6.666. 
```{r drought-rescale}
...
### MY CHANGE - CHANGING FROM 5 to 6.666 ###
# 4 * 6.666 = 20, match 20% weighting of exposition to drought events
...
```{r clip-map-drought-exposure-2, warning = F}
names(fl) <- "fl"
...
## My CHANGE - MULTIPLY BY 3.333 INSTEAD OF 5 ###
Since flood is already on scale from 0 to 4, simply multiply by 3.333 to achieve the 26.666% weight.

```{r mask-and-rescale-flood}
flood_r <- flw * mask * flood_percentage
...
I am not certain that drought should make up exactly 26.666% of the vulnerability index, and flood 13.333%. Rather, I am just pointing out the discrepancy in the precision of the rasters in two layers that are weighted the same amount in the model. On top of this, it is worth reminding the reader that the drought raster is just a population map, and not a measure of rainfall across the world.

# Conclusions

By comparing my modified final map with the original, it is clear that the weighting does make a difference to the overall vulnerability. While more could be said with statistics about how much difference, it seems that weighting the drought layer doubly conpared to flood layer increases overall vulnerability in the country.
